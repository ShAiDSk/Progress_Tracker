<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto px-4 py-10">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h2 class="text-4xl font-extrabold neon-title">
                    {{ isset($isArchived) && $isArchived ? 'Archived Goals' : 'Your Goals' }}
                </h2>

                <div class="mt-2 text-sm">
                    @if(isset($isArchived) && $isArchived)
                    <a href="{{ route('goals.index') }}" class="text-blue-400 hover:text-blue-300">← Back to Active Goals</a>
                    @else
                    <a href="{{ route('goals.archived') }}" class="text-gray-400 hover:text-white transition">📂 View Archived Goals</a>
                    @endif
                </div>
            </div>
                ...
                <article class="glass-card rounded-xl p-6 fade-in-up">
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <div class="flex gap-2 mt-2">
                                <span class="px-2 py-0.5 rounded text-xs bg-gray-700 text-gray-300">{{ $goal->category ?? 'General' }}</span>
                                <span class="px-2 py-0.5 rounded text-xs bg-blue-900 text-blue-200 uppercase">{{ $goal->priority ?? 'Medium' }}</span>
                            </div>
                        </div>
                    ...
                    <div class="mt-5 flex items-center gap-3 flex-wrap">
                        ...
                        <a href="{{ route('goals.show', $goal->id) }}"
                            class="px-3 py-1.5 rounded-md bg-gray-700 text-white hover:bg-gray-600 transition text-sm font-semibold uppercase tracking-wide">
                            View Details
                        </a>
                        ...
                        <form action="{{ route('goals.restore', $goal->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="px-3 py-1.5 rounded-md bg-green-600 text-white text-sm hover:bg-green-500 transition">
                                Restore
                            </button>
                        </form>
                        ...
                        <form action="{{ route('goals.destroy', $goal) }}" method="POST">
                            @csrf @method('DELETE')
                            <button type="submit" data-delete class="px-3 py-1.5 rounded-md bg-red-600 text-white text-sm hover:bg-red-500 transition">
                                Remove Permanently
                            </button>
                        </form>
                        ...
                        <form action="{{ route('goals.archive', $goal) }}" method="POST">
                            @csrf
                            <button type="submit" data-hide class="px-3 py-1.5 rounded-md bg-purple-600 text-white text-sm hover:bg-purple-500 transition">
                                Hide
                            </button>
                        </form>
                        ...
                        <form method="POST" action="{{ route('goals.increment', $goal) }}">
                            ...
                            <button class="px-3 py-1.5 rounded-md bg-blue-600 text-white text-sm hover:bg-blue-500 transition">
                                Add +1
                            </button>
                        </form>
                        ...
                        <form method="POST" action="{{ route('goals.done', $goal) }}">
                            @csrf
                            <button class="px-3 py-1.5 rounded-md bg-green-600 text-white text-sm hover:bg-green-500 transition">
                                Mark Done
                            </button>
                        </form>
                        @else
                        <form method="POST" action="{{ route('goals.reopen', $goal->id) }}" class="inline-block">
                            @csrf
                            <button type="submit" class="px-3 py-1.5 rounded-md bg-yellow-600 text-white hover:bg-yellow-700 transition text-sm">
                                Reopen
                            </button>
                        </form>
                        @endif
                        ...
                        <span class="muted ml-auto hidden sm:block">Status: <strong class="text-white">{{ ucfirst(str_replace('-', ' ', $status)) }}</strong></span>
                    </div>
